 ...
 */
export async function ragPipeline(userQuery, chatHistory = [], fileContext = "") {
    try {
        ...
        // Step 1: Retrieve context from Pinecone based on the user's core question
        const context = await retrieveDocuments(userQuery);
        ...
        if (context) {
            ...
            // Briefly show part of the chunks for quick audit
            console.log(`${colors.yellow}🔍 --- RETRIEVED TEXT PREVIEW ---\n${context.substring(0, 200)}...\n--- END PREVIEW ---${colors.reset}`);
        } else {
            console.log(`${colors.magenta}⚠️ [DATABASE EMPTY]: No relevant facts found. Falling back to general knowledge.${colors.reset}`);
        }
